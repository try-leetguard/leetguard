...
def has_real_resend_key():
    api_key = os.getenv('RESEND_API_KEY', '')
    return bool(api_key and api_key != 'test-resend-api-key')

class TestEmailIntegration:
    """Integration tests for email functionality."""

    @pytest.mark.skipif(
        not has_real_resend_key(),
        reason="A real RESEND_API_KEY is not set - skipping real email tests"
    )
    def test_send_verification_email_real(self):
        ...
        if not has_real_resend_key():
            pytest.skip("A real RESEND_API_KEY is not set")
...
@pytest.mark.skip(reason="Manual email setup check; run this file directly.")
